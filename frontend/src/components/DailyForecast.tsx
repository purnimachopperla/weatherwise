      ...
      <div className="flex items-center justify-between mb-3 sm:mb-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            ...
            <p className="text-[11px] sm:text-xs text-slate-400">Long range trends with thermal spectrum bands</p>
          </div>
        </div>
      </div>
        ...
        {days.map((day, idx) => {
          ...
          return (
            <motion.div
              ...
              className="p-2.5 sm:p-3 rounded-xl bg-dark-900/40 hover:bg-dark-900/80 border border-white/5 transition-all duration-200 flex items-center justify-between gap-2 sm:gap-3 text-xs"
            >
              {/* Day Name */}
              <div className="w-14 sm:w-16 font-semibold text-slate-200 truncate text-[11px] sm:text-xs">
                {idx === 0 ? 'Today' : day.day_name}
              </div>

              {/* Weather Icon & Rain */}
              <div className="flex items-center gap-1.5 sm:gap-2 w-14 sm:w-20">
                {getWeatherIconComponent(day.weather_code, 1, 'w-4 h-4 sm:w-5 sm:h-5')}
                {day.rain_probability > 0 ? (
                  <span className="flex items-center text-[9px] sm:text-[10px] text-cyan-400 font-mono">
                    <Droplets className="w-2.5 h-2.5 mr-0.5" />
                    {day.rain_probability}%
                  </span>
                ) : (
                  <span className="text-[9px] sm:text-[10px] text-slate-600 font-mono">0%</span>
                )}
              </div>

              {/* Min Temp */}
              <div className="w-8 sm:w-10 text-right font-mono text-slate-400 text-[11px] sm:text-xs">
                {formatTemperature(day.temp_min, tempUnit, 0)}
              </div>

              {/* Gradient Temperature Bar */}
              <div className="flex-1 h-1.5 sm:h-2 bg-white/5 rounded-full relative overflow-hidden min-w-[36px]">
                <div
              ...
              {/* Max Temp */}
              <div className="w-8 sm:w-10 text-left font-mono font-bold text-white text-[11px] sm:text-xs">
